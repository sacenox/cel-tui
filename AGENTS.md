...
- **Uncontrolled by default, controlled opt-in:** Focus and scroll are framework-managed by default. Providing `focused` or `scrollOffset` explicitly switches to controlled mode (app owns the state). Value (`value`/`onChange`) on TextInput is always controlled; `cursor`/`onCursorChange` optionally controls its grapheme-safe caret.
- **Stable identity:** `stateKey` attaches framework focus, scroll, and TextInput state to a continuously mounted identity across reordering. Keys are viewport-global, and absence for one successful frame ends the lifecycle. Per-layer focus is suspended/restored as overlays mount/unmount; `autoFocus` seeds only the active layer once.
...
- **Animation is opt-in:** `Spinner` and `createTicker` provide bounded cadence with explicit start/stop/dispose; the core renderer never starts a permanent frame loop.
...
- **Input** — Kitty-first keyboard input with baseline disambiguation plus opt-in progressive flags, structured press/repeat/release metadata, alternate/layout keys and associated text, mixed-stream decoding for CSI u / CSI letter / CSI tilde / recoverable legacy control bytes / ESC-prefixed Alt combos, batched SGR mouse handling, hit detection, click/scroll routing, focus traversal (Tab/Shift+Tab/Escape/Enter) with uncontrolled and controlled modes, onKeyPress bubbling from focused element through ancestors
- **TextInput** — grapheme-aware text editing (Intl.Segmenter for emoji/ZWJ), controlled or framework-managed cursor movement, stable `stateKey` identity with compatible `onChange` fallback, batched key handling with preserved edit state across stdin chunks, auto-scroll to keep cursor visible, inverted-cell cursor with native terminal cursor positioning (blinking), background fill, placeholder rendering
- **Layering** — multi-layer compositing, transparency, topmost-layer input priority
- **Terminal** — Kitty keyboard flag push/pop, mixed keyboard stream compatibility in tmux, crash cleanup (SIGINT/SIGTERM/uncaughtException), resize clear
- **Long lists** — callable `VirtualList` component with keyed variable-height measurement, bounded cell-based windowing/overscan, controlled or internal scrolling, sticky bottom, anchor compensation, and resize invalidation

`TODO.md` records the completed quality-pass backlog and is the place to add
newly reproduced issues or measured opportunities.
...
- `state-identity.ts` — Validates viewport-global `stateKey` values across layers and snapshots mounted state lifecycles. Identity kind changes remount; cleanup is committed only after a successful frame.
- `emitter.ts` — `emitBuffer()` for full renders, `emitDiff()` for differential. Both accept an optional `Theme` plus typed cursor `EmitOptions` and wrap in CSI 2026 synchronized output. The public barrel exports `emitBuffer`, `EmitOptions`, `TerminalCursorState`, and `defaultTheme` (ANSI 16 mapping); differential emission remains a runtime-internal implementation detail.
...
- `keys.ts` — Kitty-first input decoder. `decodeKeyEvents(data)` splits a raw chunk into ordered structured events, accepting enhanced CSI u (alternate keys, event phase, associated text), CSI letter / CSI tilde, recoverable legacy ASCII control bytes, ESC-prefixed Alt combos, and raw printable text. Events carry semantic `key`, exact optional `text`, phase, key codes, and reported modifiers. `parseKey(data)` remains the single-event semantic convenience wrapper; `normalizeKey()` uses canonical `ctrl+alt+shift+super+hyper+meta+<key>` order.
...
- `terminal.ts` — `ProcessTerminal` (real I/O, Kitty baseline/optional flag push and pop, raw mode, SGR mouse, crash cleanup) and `MockTerminal` (testing). The runtime prefers Kitty semantics but the parser still accepts mixed tmux/legacy keyboard encodings.
- `packages/components/src/virtual-list.ts` — Callable variable-height list instance. Measures only the active keyed window, uses prefix sums plus spacer nodes for cold rows, preserves the first visible anchor across structural edits, and bounds/release its height cache explicitly.
