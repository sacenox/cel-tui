...
describe("Markdown", () => {
  ...
  test("heading levels have different styles", () => {
    ...
    const h1 = asText(item(nodes, 0));
    const h2 = asText(item(nodes, 1));
    const h3 = asText(item(nodes, 2));
  ...
  test("paragraph with bold renders as wrapping HStack", () => {
    ...
    expect(nodes).toHaveLength(1);
    const hstack = asContainer(item(nodes, 0));
    ...
    expect(asText(item(hstack.children, 0)).content).toBe("Hello");
    expect(asText(item(hstack.children, 1)).content).toBe(" ");
    const boldNode = asText(item(hstack.children, 2));
    expect(boldNode.content).toBe("bold");
    expect(boldNode.props.bold).toBe(true);
    expect(asText(item(hstack.children, 3)).content).toBe(" ");
    expect(asText(item(hstack.children, 4)).content).toBe("world");
  });
  ...
  test("paragraph with italic renders styled nodes", () => {
    ...
    const italicNode = asText(item(hstack.children, 2));
  ...
  test("paragraph with inline code keeps code span atomic", () => {
    ...
    expect(hstack.children).toHaveLength(5);
    const codeNode = asText(item(hstack.children, 2));
  ...
  test("paragraph with link keeps link text atomic", () => {
    ...
    expect(hstack.children).toHaveLength(5);
    const linkNode = asText(item(hstack.children, 2));
  ...
  test("paragraph with mixed formatting", () => {
    ...
    expect(asText(item(hstack.children, 0)).content).toBe("Hello,");
    expect(asText(item(hstack.children, 1)).content).toBe(" ");
    expect(asText(item(hstack.children, 2)).props.bold).toBe(true);
  ...
  test("code block renders as VStack with Text", () => {
    ...
    expect(nodes).toHaveLength(1);
    const container = asContainer(item(nodes, 0));
  ...
  test("blockquote renders with bar and italic content", () => {
    ...
    // Bar
    const bar = asText(item(row.children, 0));
  ...
  test("blank line renders as empty Text", () => {
    ...
    expect(textContent(item(nodes, 1))).toBe("");
  });
  ...
  test("custom theme partial — unset properties use defaults", () => {
    ...
    // Heading uses custom
    expect(asText(item(nodes, 0)).props.fgColor).toBe("color01");
    // Blockquote bar uses default
    const row = asContainer(item(nodes, 1));
  ...
  test("link text is used, url is stripped", () => {
    ...
    // "See", " ", link("the docs"), " ", "here."
    expect(hstack.children).toHaveLength(5);
    expect(asText(item(hstack.children, 0)).content).toBe("See");
    const link = asText(item(hstack.children, 2));
    expect(link.content).toBe("the docs");
    expect(link.props.fgColor).toBe("color06");
    expect(asText(item(hstack.children, 4)).content).toBe("here.");
  });
  ...
  test("list item with inline formatting", () => {
    ...
    const content = asContainer(item(wrapper.children, 0));
    ...
    expect(asText(item(content.children, 2)).props.bold).toBe(true);
  });
